/**
 * Error handling utilities for dynamic branding in web-next
 * Provides graceful fallbacks for API failures and other branding errors
 */

import { createSafeWebTitle, createLocalizedWebTitle } from './titleUtils';
...
export function createSafeBrandingConfig(
  ...
  // Handle title with fallbacks
  let title: string;
  try {
    title = createLocalizedWebTitle(pageTitle, clinicName, locale);
  } catch (error) {
    const brandingError: BrandingError = {
      type: BrandingErrorType.TITLE_UPDATE_FAILED,
      message: 'Failed to create web title, using fallback',
      originalError: error instanceof Error ? error : new Error('Unknown title error'),
      timestamp: new Date()
    };
    errors.push(brandingError);
    if (onError) onError(brandingError);
    
    // Absolute fallback based on locale
    title = locale === 'ar' ? 'العيادة | الرئيسية' : 'Clinic | Home';
  }
...
/**
 * Create safe branding configuration for clinic settings
 * 
 * @param pageTitle - Current page title
 * @param clinicSettings - Full clinic settings object
 * @param locale - Current locale
 * @param onError - Optional error callback
 * @returns Safe branding configuration
 */
export function createSafeBrandingFromSettings(
  pageTitle?: string | null,
  clinicSettings?: any,
  locale: string = 'en',
  onError?: (error: BrandingError) => void
  ...
  try {
    title = monitorTitleUpdatePerformance(
      ...
      locale,
      () => createLocalizedWebTitle(pageTitle, clinicSettings?.clinicName, locale)
    );
  } catch (error) {
    const brandingError: BrandingError = {
      type: BrandingErrorType.TITLE_UPDATE_FAILED,
...
export function handleApiFailure(
  ...
  const fallbackTitle = locale === 'ar' ? 'العيادة | الرئيسية' : 'Clinic | Home';
 ...
 * 
 * @param pageTitle - The page title being updated
 * @param clinicName - The clinic name being used
...
/**
 * React hook for safe branding with error handling
 * 
 * @param pageTitle - Current page title
...
export function useSafeBranding(
  ...
  try {
    return createSafeBrandingFromSettings(
      pageTitle,
      clinicSettings,
      locale,
      logBrandingError
    ...
    return handleApiFailure(
      error instanceof Error ? error : new Error('Unknown branding error'),
      locale,
      logBrandingError
    );
  }
