...
public class PlanCatalogController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PlanCatalogController.class);

    private final PayPalConfigService payPalConfigService;
    private final PlanTierConfig planTierConfig;

    public PlanCatalogController(PayPalConfigService payPalConfigService, PlanTierConfig planTierConfig) {
        this.payPalConfigService = payPalConfigService;
        this.planTierConfig = planTierConfig;
    }

    @GetMapping
    @Operation(summary = "List public plans", description = "Returns available plans with pricing, features, and PayPal plan IDs")
    public ResponseEntity<List<PublicPlanDto>> listPlans(@RequestParam(defaultValue = "USD") String currency) {
        ...
        logger.info("Listing public plans for currency {}", normalizedCurrency);

        List<PayPalPlanConfigDto> configs = new ArrayList<>();
        try {
            configs = payPalConfigService.getPlanConfigsList();
        } catch (Exception e) {
            logger.warn("No PayPal plan configs found; using defaults", e);
        }
        ...
        for (PayPalPlanConfigDto config : configs) {
            ...
            try {
                ...
                logger.warn("Skipping unknown plan tier returned from config: {}", tierKey);
                continue;
            }
        ...
        if (results.isEmpty()) {
            logger.warn("No PayPal plan configs found; falling back to PlanTierConfig defaults");
