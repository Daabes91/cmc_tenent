...
const currentPageTitle = computed(() => {
  ...
  const titleMap: Record<string, string> = {
    ...
    '/treatment-plans': t('navigation.treatmentPlans'),
    '/materials': t('navigation.materials'),
    '/services': t('navigation.services'),
    '/insurance-companies': t('navigation.insuranceCompanies'),
    '/blogs': t('navigation.blogs'),
    '/reports': t('navigation.reports'),
    '/staff': t('navigation.staff'),
    '/clinic-settings': t('navigation.clinicSettings'),
...
const searchInputUi = computed(() => ({
  ...
  base: isDark.value
    ? "bg-white/5 border-white/10 text-slate-100 placeholder:text-slate-400 focus:ring-2 focus:ring-violet-500/60 text-base"
    : "bg-white/90 border-slate-300 text-slate-900 placeholder:text-slate-500 focus:ring-2 focus:ring-violet-500/20 text-base",
  wrapper: "relative",
  padding: "ps-10 pe-12"
}));
...
watch(
  () => route.path,
  () => {
    // Mobile sidebar is handled by useMobileNavigation composable
    search.value = "";
    closeSearchPanel();
  }
);
